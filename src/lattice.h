...
// lattice.h
//
//  - Lattice data structures for lb3d_prime.
//
...
struct pdf_struct
{
  double ftemp[ /*NUM_DIRS*/ 19];
};
#else
struct pdf_struct
{
  double feq[   /*NUM_DIRS*/ 19];
  double f[     /*NUM_DIRS*/ 19];
  double ftemp[ /*NUM_DIRS*/ 19];
...
struct force_struct
{
  double force[  /*NUM_DIMS*/3];
  double sforce[ /*NUM_DIMS*/3];
};
...
struct process_struct
{
  ...
  double *z_pos_pdf_to_send;
  double *z_pos_pdf_to_recv;
  double *z_neg_pdf_to_send;
  double *z_neg_pdf_to_recv;

  double *z_pos_rho_to_send;
  double *z_pos_rho_to_recv;
  double *z_neg_rho_to_send;
  double *z_neg_rho_to_recv;
...
struct param_struct
{
  ...
  //
  double gforce[   NUM_FLUID_COMPONENTS][3];
  ...
  // Type: double big_V0_solid[NUM_FLUID_COMPONENTS];
  ...
  double rel_x1, rel_x2, rel_z1;
  double rel_y1, rel_y2, rel_z2;
...
struct lattice_struct
{
  ...
  struct solids_struct     *solids[     NUM_FLUID_COMPONENTS];
#if NON_LOCAL_FORCES
